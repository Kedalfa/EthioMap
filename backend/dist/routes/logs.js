...
router.get('/', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const countResult = await pool.query(`SELECT COUNT(*) FROM activity_logs ${where}`, values);
        const total = parseInt(countResult.rows[0].count, 10);
        const logsResult = await pool.query(`SELECT id, user_id, username, action, resource_type, resource_id,
...
router.get('/stats', requireAuth, requireAdmin, async (_req, res) => {
    try {
        const [datasets, users, logs, recentLogs] = await Promise.all([
            ...
            pool.query(`SELECT username, action, resource_name, created_at
