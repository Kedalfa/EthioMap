...
export function signToken(user: AuthUser): string {
  const expiresIn = (process.env.JWT_EXPIRES_IN || '8h') as `${number}${'s'|'m'|'h'|'d'|'w'}` | number;
  return jwt.sign(
...
export function getClientIp(req: Request): string {
  const raw = (req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || req.socket.remoteAddress || '127.0.0.1';
