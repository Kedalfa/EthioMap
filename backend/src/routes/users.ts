...
router.get('/', requireAdmin, async (_req: Request, res: Response) => {
  try {
    const result = await pool.query(
      `SELECT id, username, email, role, is_active, created_at, avatar_url
...
router.post('/', requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [String(username).trim(), String(email).trim().toLowerCase(), passwordHash, userRole, req.user!.id]
    );
  ...
  try {
    const result = await pool.query(
      `UPDATE users
       ...
       WHERE id = $4
       RETURNING id, username, email, role, is_active, created_at`,
      [role || null, allowedRoles, is_active !== undefined ? is_active : null, req.params.id]
    );
