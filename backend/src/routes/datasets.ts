...
// Interface representing the expected body of the POST request
interface DatasetUploadRequest {
  filename?: string;
  name?: string;
  geojson: any; // Checked and normalized inside normalizeGeoJSON
  metadata?: any;
}

// Interface for dataset summary response
...
function normalizeGeoJSON(input: any): FeatureCollection {
 ...
 */
function detectSRID(geojson: FeatureCollection, metadata?: any): number {
  // 1. Sample the first coordinate to check empirical value range.
  const firstFeature = geojson.features?.[0];
  const geom: any = firstFeature?.geometry;
  let coord: number[] | undefined;
  if (geom?.type === 'Point') coord = geom.coordinates;
  else if (geom?.type === 'MultiPoint') coord = geom.coordinates?.[0];
  else if (geom?.type === 'LineString') coord = geom.coordinates?.[0];
  else if (geom?.type === 'MultiLineString') coord = geom.coordinates?.[0]?.[0];
  else if (geom?.type === 'Polygon') coord = geom.coordinates?.[0]?.[0];
  else if (geom?.type === 'MultiPolygon') coord = geom.coordinates?.[0]?.[0]?.[0];
  else if (geom?.type === 'GeometryCollection') {
    const sub: any = geom.geometries?.[0];
    if (sub?.type === 'Point') coord = sub.coordinates;
    else coord = sub?.coordinates?.[0]?.[0];
  }
  ...
  const geojsonCrs = String((geojson as any)?.crs?.properties?.name || '').toUpperCase();
  const combinedCrs = `${userCrs} ${geojsonCrs}`;

  if (combinedCrs.includes('20137') || combinedCrs.includes('32637') || combinedCrs.includes('37N')) return 32637;
  if (combinedCrs.includes('20138') || combinedCrs.includes('32638') || combinedCrs.includes('38N')) return 32638;
  if (combinedCrs.includes('20136') || combinedCrs.includes('32636') || combinedCrs.includes('36N')) return 32636;
...
function datasetSummary(row: any): DatasetSummary {
  return {
    id: row.id,
    name: row.name,
...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const result = await pool.query(
      'SELECT id, name, original_filename, feature_count, metadata, created_at FROM datasets ORDER BY created_at DESC'
    );
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const featureCollection: FeatureCollection = {
      type: 'FeatureCollection',
      features: featuresResult.rows.map((row: any) => ({
        type: 'Feature',
...
router.post('/', requireAuth, async (req: Request<{}, {}, DatasetUploadRequest>, res: Response) => {
  let client;
  try {
...
router.put('/:id', requireAuth, async (req: Request<{ id: string }, {}, { name?: string; metadata?: any }>, res: Response) => {
  try {
